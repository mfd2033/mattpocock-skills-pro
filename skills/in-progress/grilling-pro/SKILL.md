...
- **One call takes at most 4 questions.** A round carries the whole frontier, so a round of five or more questions runs as consecutive calls with continuous numbering. The round is the run of calls, not one call.
...
- **Say the shape above the call, then send it**: one line, like `4 questions, 3/3/3/2 options each`, in the session's language. If a number you just wrote is past three, fix that question before sending; the count is the check.
- **At most three options per question, every one a real alternative.** The panel fits about four rows, and the harness always renders a typed-input row of its own below the options, so the options get three: a fourth option is the row that pushes the input box below the fold, and the human answers from what they can see. `自定义` / `Other` is never among them: it would duplicate the input row the harness already draws. When the real alternatives exceed three, put the strongest three in and name the rest in the question body (the human can type any of them), or split the question into a narrowing step and a comparison step when three would misrepresent the decision. Completeness is preserved by the body and the typed answer, never by a scrolled panel.
- Turn multi-select on only where the decision genuinely admits several answers at once; a decision with one right answer stays single-select.
- **Typing stays open.** The options are a shortcut, so the human can always answer in the chat instead.
- **The chat is the later signal.** The human may answer in the panel and then keep talking in the chat. Where the chat message adds to a pick, take both. Where it contradicts a pick, it wins because it came later: say so in one line and carry on with the round.
